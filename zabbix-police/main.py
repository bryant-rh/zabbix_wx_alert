...
def send(alert_type):
    try:
        ...
        #获取token
        token_1 = GetToken(CorpID,Secret_1)
        #日志告警群
        token_2= GetToken(CorpID,Secret_2)
        #发送告警消息
        if len(content) != 0:
            for i in content:
                event = i['event']
                ...
                item_key = message.strip().split('监控项目:')[1].split('\n')[0].strip()
                if "/data/logs/nh_service/national-service-error.log" in item_key:
                    ...
                    message_log = re.sub(r'告警详情.*$','告警详情: <a href="http://monitor.hitales.ai:8887">点击查看日志系统</a>',message)
                    res = SendTextMessage(token_2,Users,Agentid_2,message_log.strip())
                else:
                    res = SendTextMessage(token_1,Users,Agentid_1,message.strip())
                res_dict = json.loads(res)
                if res_dict['errcode'] == 0:
                    if alert_type == 'alert':
                        server_loger.info('Send Alert message success: event: %s, item_key: %s, %s' % (event, item_key, res_dict))
                    else:
                        server_loger.info('Send Recovery message success: event %s, item_key: %s, %s' % (event, item_key, res_dict))
                    for key in event:
                        delete_redis(redis_host,redis_port,key)
                        
                else:
                    if alert_type == 'alert':
                        server_loger.error('Send Alert message error: event: %s, item_key: %s, %s' % (event, item_key, res_dict))
                    else:
                        server_loger.error('Send Recovery message error: event: %s, item_key: %s, %s' % (event, item_key, res_dict))
